...
import com.feytuo.chat.applib.controller.HXSDKHelper;
...
public class SettingsFragment extends Fragment implements OnClickListener {
	...
	public void onClick(View v) {
		...
		if(v.getId() == R.id.rl_switch_sound){
		...
		if(v.getId() == R.id.rl_switch_vibrate){
		...
		if(v.getId() == R.id.rl_switch_speaker){
		...
		if(v.getId() == R.id.btn_logout){ //退出登陆
			logout();
		}
		if(v.getId() == R.id.ll_black_list){
			startActivity(new Intent(getActivity(), BlacklistActivity.class));
		}
		if(v.getId() == R.id.ll_diagnose){
...
//        if(((Fragment4) getParentFragment()).isConflict){
//        	outState.putBoolean("isConflict", true);
//        }else if(((Fragment4) getParentFragment()).getCurrentAccountRemoved()){
